namespace System.Management.Automation
{
    ...
    internal class FaqHelpProvider : HelpProviderWithFullCache
    {
        ...
        private void LoadFaqDiv(System.Xml.XmlNode xmlNode)
        {
            if (xmlNode != null)
            {
            ...
            if (!string.IsNullOrEmpty(helpFile))
            {
                ...
                if (document.HasChildNodes)
                {
                    for (int i = 0; i < document.ChildNodes.Count; i++)
                    {
                        ...
                        if ((node2.NodeType == XmlNodeType.Element) && (string.Compare(node2.Name, "faq", StringComparison.OrdinalIgnoreCase) == 0))
                        {
                            node = node2;
                            break;
                        }
                    ...
                    using (base.HelpSystem.Trace(helpFile))
                    {
                        if (node.HasChildNodes)
                        {
                            for (int j = 0; j < node.ChildNodes.Count; j++)
                            {
                                ...
                                if ((xmlNode.NodeType == XmlNodeType.Element) && (string.Compare(xmlNode.Name, "faqEntry", StringComparison.OrdinalIgnoreCase) == 0))
                                {
                                    HelpInfo helpInfo = null;
                                    helpInfo = FaqHelpInfo.Load(xmlNode);
                                    if (helpInfo != null)
                                    {
                                        base.HelpSystem.TraceErrors(helpInfo.Errors);
                                        base.AddCache(helpInfo.Name, helpInfo);
                                ...
                                else if ((xmlNode.NodeType == XmlNodeType.Element) && (string.Compare(xmlNode.Name, "faqDiv", StringComparison.OrdinalIgnoreCase) == 0))
                                {
                                    this.LoadFaqDiv(xmlNode);
                                }
                            }
                        }
                    }
