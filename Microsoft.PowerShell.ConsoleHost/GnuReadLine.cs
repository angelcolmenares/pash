...
namespace Mono.Terminal {
	...
	public class LineEditor {
		...
		IExternalHistory externalHistory;
		...
		void ComputeRendered ()
		{
			...
			for (int i = 0; i < text.Length; i++){
				...
				if (c < 26){
					...
					rendered_text.Append (SecureChar.GetValueOrDefault ());
				}
				else {
		...
		void CmdBackspace ()
		{
			if (externalHistory != null) externalHistory.ResetFilter ();
			if (cursor == 0)
				return;
		...
		void CmdHistoryPrev ()
		{
			if (externalHistory != null) {
				string historyText = externalHistory.Previous (text.ToString ());
		...
		{
			if (externalHistory != null) {
				string historyText = externalHistory.Next (text.ToString ());
				if (!string.IsNullOrEmpty (historyText))
				{
					SetText (historyText);
					return;
		...
		public interface IExternalHistory
		{
			string Previous(string currentText);

			string Next(string currentText);
