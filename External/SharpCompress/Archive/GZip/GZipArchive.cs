...
namespace SharpCompress.Archive.GZip
{
    public class GZipArchive : AbstractWritableArchive<GZipArchiveEntry, GZipVolume>
    {
		...
		protected override GZipArchiveEntry CreateEntry(int index, string filePath, Stream source, long size, DateTime? modified)
        {
            if (Entries.Any())
            {
                throw new InvalidOperationException("Only one entry is allowed in a GZip Archive");
            }
            return new GZipWritableArchiveEntry(index, this, source, filePath, size, modified);
        }
        ...
        protected override IEnumerable<GZipArchiveEntry> LoadEntries(IEnumerable<GZipVolume> volumes)
        {
            ...
            yield return new GZipArchiveEntry(1, this, new GZipFilePart(stream));
        }
