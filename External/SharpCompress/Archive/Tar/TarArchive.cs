...
namespace SharpCompress.Archive.Tar
{
    public class TarArchive : AbstractWritableArchive<TarArchiveEntry, TarVolume>
    {
            ...
            foreach (TarHeader header in TarHeaderFactory.ReadHeader(StreamingMode.Seekable, stream))
            {
                if (header != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var entry = new TarArchiveEntry(1, this, new TarFilePart(previousHeader, stream), CompressionType.None);
                        ...
                        yield return new TarArchiveEntry(i++, this, new TarFilePart(header, stream), CompressionType.None);
                    }
                }
            }
        }
        ...
        protected override TarArchiveEntry CreateEntry(int index, string filePath, Stream source,
            long size, DateTime? modified)
        {
            return new TarWritableArchiveEntry(index, this, source, CompressionType.Unknown, filePath, size, modified);
        }
