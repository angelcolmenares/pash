...
namespace System.Runtime
{
	internal abstract class AsyncResult : IAsyncResult
	{
		...
		protected void Complete(bool completedSynchronously)
		{
			...
			{
				throw new InvalidOperationException("AsyncResultCompletedTwice"); // Fx.Exception.AsError(new InvalidOperationException(InternalSR.AsyncResultCompletedTwice(this.GetType())));
			}
		}
			...
			if (result != null)
			{
				...
				if (tAsyncResult != null)
				{
					if (!tAsyncResult.endCalled)
					{
						...
						{
							throw tAsyncResult.exception; //TODO: Fx.Exception.AsError(tAsyncResult.exception);
						}
					}
					else
					{
						throw new InvalidOperationException("AsyncResultAlreadyEnded"); //TODO: Fx.Exception.AsError(new InvalidOperationException(InternalSR.AsyncResultAlreadyEnded));
					}
				}
				else
				{
					throw new ArgumentException("result"); //TODO: Fx.Exception.Argument("result", InternalSR.InvalidAsyncResult);
				}
			}
			else
			{
				throw new ArgumentNullException("result"); //TODO: Fx.Exception.ArgumentNull("result");
			}
		}
		...
		{
			throw new InvalidOperationException("InvalidAsyncResultImplementation"); //TODO: Fx.Exception.AsError(new InvalidOperationException(InternalSR.InvalidAsyncResultImplementation(result.GetType())));
		}

		protected static void ThrowInvalidAsyncResult(string debugText)
		{
			string invalidAsyncResultImplementationGeneric = "InvalidAsyncResultImplementationGeneric"; //TODO: InternalSR.InvalidAsyncResultImplementationGeneric;
			throw new InvalidOperationException(invalidAsyncResultImplementationGeneric); //TODO: Fx.Exception.AsError(new InvalidOperationException(invalidAsyncResultImplementationGeneric));
		}
